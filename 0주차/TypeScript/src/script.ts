...
const todoInput = document.getElementById('todo-input') as HTMLInputElement; //as 타입
...
const renderTasks = ():void => { //매개변수x, 리턴x
    todoList.innerHTML = '';
    doneList.innerHTML = '';
...
const getTodoText = (): string => { //매개변수x, 리턴 타입 string
    return todoInput.value.trim();
};

// 4. 할 일 추가 처리 함수

const addTodo = (text: string): void => { //매개변수타입 string, 리턴x
    todos.push({id: Date.now(), text}); //text: text
    todoInput.value = ''; //입력했으면 input 창에서 지우기
    renderTasks();
};
...
const createTodoElement = (todo: Todo, isDone: boolean): HTMLElement => {
    ...
    return li;
                ...
                //         <p class="render-container__item_text">123</p>
